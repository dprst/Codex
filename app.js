...
const t = {
  uk: { today: 'Сьогодні', archive: 'Архів', method: 'Методологія', topSignals: '3 ключові сигнали', executive: 'Executive summary (1 хв)', global: '🌍 World Digest', ukraine: '🇺🇦 Україна', pro: '📣 Communications Strategy', research: '📚 Рекомендовано', metrics: '📊 Метрики', sourceTitle: 'Джерела', methodTitle: 'Методологія', toc: 'Навігація', all: 'Усі теми', why: 'Чому важливо', implication: 'Імплікації', sources: 'Першоджерела', open: 'Відкрити', updated: 'Оновлено', datePrefix: 'Випуск', details: 'Детальніше', loadError: 'Не вдалося завантажити випуск.', methodBullets: ['Кожна новина має гіперпосилання на першоджерело.', 'Випуск ділиться на: World, Ukraine, Communications Strategy, Metrics, Research.', 'Контент подається як short summary + deeper context у розгортанні.', 'Список джерел дублюється окремим блоком внизу випуску.'] },
  en: { today: 'Today', archive: 'Archive', method: 'Method', topSignals: 'Top 3 signals', executive: 'Executive summary (1 min)', global: '🌍 World Digest', ukraine: '🇺🇦 Ukraine', pro: '📣 Communications Strategy', research: '📚 Recommended', metrics: '📊 Metrics', sourceTitle: 'Sources', methodTitle: 'Methodology', toc: 'Jump to', all: 'All categories', why: 'Why it matters', implication: 'Comms implication', sources: 'Primary sources', open: 'Open', updated: 'Updated', datePrefix: 'Issue', details: 'Read more', loadError: 'Failed to load issue.', methodBullets: ['Every story includes direct hyperlinks to primary sources.', 'Each issue is split into: World, Ukraine, Communications Strategy, Metrics, Research.', 'Content uses short summaries with expandable deeper context.', 'All sources are also listed in a dedicated source section.'] }
};
...
function storyCard(story) {
  return `<article class="story"><h3>${safe(story.headline?.[state.lang], safe(story.headline?.uk, ''))} <span class="tag ${story.importance || 'watch'}">${story.importance || 'watch'}</span></h3><div class="meta">${story.category || ''} • ${story.score || '-'} /100 • ${story.published_at || ''}</div><p>${safe(story.summary?.[state.lang], '')}</p><details><summary>${text('details')}</summary><p><span class="label">${text('why')}:</span> ${safe(story.why_it_matters?.[state.lang], '')}</p><p><span class="label">${text('implication')}:</span> ${safe(story.comms_implication?.[state.lang], '')}</p></details><div class="sources"><span class="label">${text('sources')}:</span> ${links(story.sources)}</div></article>`;
}
...
function renderIssue() {
  ...
  dom.proList.innerHTML = safe(issue.pro_block?.items, []).map((p) => `<article class="pro-item"><h3>${safe(p.title?.[state.lang], '')}</h3><p>${safe(p.insight?.[state.lang], '')}</p><p><span class="label">${text('implication')}:</span> ${safe(p.application?.[state.lang], '')}</p><div class="sources"><span class="label">${text('sources')}:</span> ${links(p.sources)}</div></article>`).join('');

  dom.metricsList.innerHTML = safe(issue.comms_metrics, []).map((m) => `<article class="metric-item"><h3>${safe(m.title?.[state.lang], '')}</h3><p>${safe(m.description?.[state.lang], '')}</p><p><span class="label">Formula:</span> <code>${safe(m.formula, '')}</code></p><ul>${safe(m.tools, []).map((tool) => `<li><a href="${tool.url}" target="_blank" rel="noopener">${tool.name}</a></li>`).join('')}</ul></article>`).join('');

  dom.researchList.innerHTML = safe(issue.research_radar, []).map((r) => `<article class="research-item"><h3>${r.title}</h3><p>${safe(r.why_read?.[state.lang], '')}</p><div class="sources"><a href="${r.url}" target="_blank" rel="noopener">${text('open')}</a></div></article>`).join('');
  ...
  const toc = [['#executive', text('executive')], ['#signalsSection', text('topSignals')], ['#globalSection', text('global')], ['#ukraineSection', text('ukraine')], ['#proSection', text('pro')], ['#metricsSection', text('metrics')], ['#researchSection', text('research')], ['#sourceSection', text('sourceTitle')]];
...
function renderArchive() {
  ...
  dom.archiveList.innerHTML = filtered.map((i) => `<li><div><strong>${i.meta.date}</strong><br><small>${safe(i.editorial_intro?.[state.lang], '').slice(0, 110)}…</small></div><button class="open-issue" data-date="${i.meta.date}">${text('open')}</button></li>`).join('');
}
